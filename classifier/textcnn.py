...
def main():
    ...
    parser.add_argument('-device', default='cpu', type=str, help='set the device for training')
    ...
    if os.path.exists('./checkpoints/embedding.pt'):
        ...
        embed_path = './char-embeddings/glove.840B.300d-char.txt'
        ...
        for idx, batch in enumerate(train_loader):
            ...
            if optimizer.steps % opt.log_step == 0:
                ...
                print('[Info] Epoch {:02d}-{:05d}: | average acc {:.4f}% | '
                    'average loss {:.4f} | lr {:.6f} | second {:.2f}'.format(e, optimizer.steps, total_acc / total_num * 100,
                                                                            #  np.mean(loss_list, lr, time.time() - start)))
                                                                             np.mean(loss_list), np.mean(lr), np.mean(time.time()-start)))
                start = time.time()
